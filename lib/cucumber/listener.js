var Listener = function () {
  ...
  function appendEventHandlers(target) {
    var Events = Listener.Events;

    for(event in Events) {
      if(Events.hasOwnProperty(event))
        appendEventHandler(Events[event], target)
    }
  }

  function appendEventHandler(shortName, target) {
    target[shortName] = function(handler) { target.setHandlerForEvent(shortName, handler); };
  }

  appendEventHandlers(self);
  return self;
};
